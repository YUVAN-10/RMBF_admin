  ...
  return (
    <div className="flex flex-col items-center justify-between gap-3 border-t border-border pt-4 sm:flex-row">
      <p className="text-sm text-text-secondary">
        Showing <span className="font-medium text-text">{rangeStart}-{rangeEnd}</span> of <span className="font-medium text-text">{totalItems}</span> thank notes
      </p>
        ...
        <div className="flex items-center gap-1">
          {pages.map((page) => (
            <button
              ...
              className={
                page === currentPage
                  ? "h-8 w-8 rounded-lg bg-primary text-white"
                  : "h-8 w-8 rounded-lg text-sm font-medium text-text-secondary transition-colors hover:bg-bg"
              }
            >
              {page}
            </button>
